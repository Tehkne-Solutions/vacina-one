  ...
  return (
    <header className="fixed left-0 top-0 z-[100] w-full border-b border-[#EAF4EB] bg-white/95 shadow-[0_2px_10px_rgba(0,0,0,0.05)] backdrop-blur">
      <div className="mx-auto flex h-[72px] w-[85%] items-center justify-between md:h-[88px]">
        <Link href="/" aria-label="Ir para a página inicial da VacinaOne" className="relative h-[30px] w-[132px] shrink-0 md:h-[38px] md:w-[160px]">
          <Image src="/images/vacina-one-logo.png" alt="VacinaOne" fill className="object-contain" priority />
        </Link>

        <nav className="hidden items-center gap-4 font-franie text-[13px] font-semibold text-[#1A3858] xl:flex 2xl:gap-6 2xl:text-[14px]">
        ...
        <a
          ...
          className="hidden h-[40px] items-center justify-center rounded-[13px] border border-[#56B0BB] px-5 font-franie text-[13px] font-bold text-[#56B0BB] transition-all duration-300 hover:bg-[#56B0BB] hover:text-white xl:inline-flex"
        >
          Seja um Franqueado
        </a>
      ...
      <div className={`overflow-hidden bg-white shadow-xl transition-all duration-300 xl:hidden ${isOpen ? 'max-h-[520px] opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className="mx-auto flex w-[85%] flex-col gap-1 py-3">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="mt-3 inline-flex h-[42px] w-full items-center justify-center rounded-[13px] bg-[#56B0BB] font-franie text-[15px] font-bold text-white transition-all duration-300 hover:brightness-105"
          >
            Seja um Franqueado
          </a>
        </div>
